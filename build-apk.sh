...
GENERATED_VARIANT_FILE="$MOBILE_DIR/src/generated/app-variant.js"

write_default_embedded_variant_module() {
  mkdir -p "$(dirname "$GENERATED_VARIANT_FILE")"
  cat > "$GENERATED_VARIANT_FILE" <<'EOF'
...
write_embedded_variant_module() {
  local variant="$1"

  mkdir -p "$(dirname "$GENERATED_VARIANT_FILE")"
  cat > "$GENERATED_VARIANT_FILE" <<EOF
// Auto-generated by build-apk.sh for Android variant builds.
// This file is overwritten during builds and reset after the script exits.

export const EMBEDDED_APP_VARIANT = '${variant}';
export default EMBEDDED_APP_VARIANT;
EOF
}

trap 'write_default_embedded_variant_module >/dev/null 2>&1 || true' EXIT
write_default_embedded_variant_module
...
for variant in "${APP_VARIANTS[@]}"; do
  ...
  info "Embedding runtime app variant for $variant"
  write_embedded_variant_module "$variant"
