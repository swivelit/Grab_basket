...
function normalizeRouteLeg({ leg, label, index, source }) {
  const distanceMeters = Number(
    leg?.distanceMeters ?? leg?.distance?.value ?? 0
  );
...
async function fetchDirectionsApiRoute(plan, apiKey) {
  ...
  if (plan?.waypoints?.length) {
    params.set(
      'waypoints',
      `optimize:false|${plan.waypoints.map((point) => formatLatLng(point)).join('|')}`
    );
  }
  ...
  if (payload?.status !== 'OK') {
    throw createHttpError(payload?.error_message || payload?.status || 'Directions API did not return a route.', {
      status: response.status,
      payload,
    });
  }
...
export default function DeliveryControlCenter() {
  ...
  const routePlan = useMemo(
    ...
    [activeOrder?.status, dropPoint, pickupPoint, riderPoint]
  );
      ...
      <ScrollView
        ...
        <View style={styles.screenHeader}>
          ...
          <Text style={styles.screenSubtitle}>
            Push assignment handling, live GPS tracking, real directions-based route preview, filters,
        ...
        <SectionCard
          ...
          subtitle="Background GPS is wired to the existing /partner/location endpoint and the route card now resolves a real route with ETA instead of a straight pickup-drop line."
          right={
            <View
              style={[
                styles.inlineBadge,
          ...
          <View style={styles.mapWrap}>
            {Platform.OS === 'web' ? (
              <View style={styles.webMapFallback}>
                ...
                <Text style={styles.webMapFallbackTitle}>Map preview is only available on iOS and Android.</Text>
                <Text style={styles.webMapFallbackSubtitle}>
                  The rider app will still open the active stop in the installed maps application.
                </Text>
              </View>
            ) : (
              <>
          ...
          <View style={styles.metaList}>
            ...
            <MetaLine
              ...
              label={
                routePlan
                  ? `Preview flow: ${routePlan.summaryLabel}`
                  : 'Route preview will appear when pickup/drop coordinates are available.'
              }
            />
          </View>
